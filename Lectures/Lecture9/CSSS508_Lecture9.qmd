...
#| error: true
airquality |> 
  summarise(across(Ozone:Temp, median())) # <1>
```
1. This error arises because you’re calling the function with no input, `i.e. median()`.
...
```{r}
airquality |> 
  summarise(across(Ozone:Temp, \(x) median(x, na.rm = TRUE))) # <1>
```
1. So-called anonymous, because we never explicitly gave it a name with `<-`. Another term programmers use for this is “lambda function”.
...
```{r}
airquality |> 
  summarise(across(Ozone:Temp, ~ median(.x, na.rm = TRUE))) # <2>
```
2. This is another way to write anonymous functions but it only works inside tidyverse functions and always uses the variable name `.x`. Base syntax is now recommended (i.e. `\(x) x + 1`).
...
airquality |> 
  summarise(across(Ozone:Temp, 
                   list(median = \(x) median(x, na.rm = TRUE), # <1> 
                        n_miss = \(x) sum(is.na(x))))) # <2> 
```
1. The names of the list are used to name the new variables. In fact, the columns are named using a glue specification `{.col}_{.fn}` where `.col` is the name of the original column and `.fn` is the name of the function.
2. Anonymous functions are also necessary if nesting functions such as in this example.

## Column Names

[By default, the output of `across()` is given the same names as the inputs. This means that `across()` inside of mutate() will replace existing columns.]{style="font-size: 85%;"}
...
[If you’d like to instead create new columns, you can use the `.names` argument to give the output new names.]{style="font-size: 85%;"} 

. . . 

<div class=smaller-code>
```{r}
df |> mutate(across(a:d, rescale01, .names = "{.col}_rescaled")) # <1>
```
1. `.col` simply represents the original variable name. 
</div>
...
```{r}
airquality |> filter(if_any(Ozone:Temp, is.na)) # <1>
```
1. This is the same as `airquality |> filter(is.na(Ozone) | is.na(Solar.R) | is.na(Wind) | is.na(Temp))`

```{r}
airquality |> filter(if_all(Ozone:Temp, is.na)) # <2>
```
2. This is the same as `airquality |> filter(is.na(Ozone) & is.na(Solar.R) & is.na(Wind) & is.na(Temp))`

## `across()` in Functions

Naturally, `across()` lends itself to use within user-defined functions because it allows you to operate on multiple columns simultaneously. 
...
```{r}
summarize_means <- function(df, summary_vars = where(is.numeric)) { # <1>
  df |> # <1>
    summarize( # <1>
      across({{ summary_vars }}, \(x) mean(x, na.rm = TRUE)), # <1>
      n = n(), # <1>
      .groups = "drop" # <1>
    ) # <1>
} # <1>
```
1. A function that takes a dataframe and (*optionally*) variable names and returns an ungrouped dataframe with the mean of each of those variables (with `NA`s removed) as well as the count of observations. If no variables are specified, it defaults to all numeric variables. <br><br> If the dataframe is grouped, the summary statistics will be evaluated by the values of the grouping variable(s). 
...
Instead of listing out all the `read_excel()` calls in our list, we can used the `map()` function from the tidyverse's `purrr` package. `map()` is similar to `across()`, but instead of modifying columns in a data frame, it does something to each element of a list/vector.
...
files[[1]] # <1>
```
1. The first element of `files` is a `tibble`, meaning `files` is a list of all the datasets we told it to read in. 
:::
::: {.column width="70%"}
::: {.fragment}
```{r}
...
files <- paths |> 
  set_names(basename) |> # <1>
  map(readxl::read_excel)
```
1. The `set_names` function takes the function `basename` which extracts just the file name from the full path. This line of code will therefore create a named vector of the file paths where the names are actually the filenames. 
...
[Now each of our dataframes is named for the year of data it contains.]{.fragment fragment-index=2 style="font-size: 75%;"}

::: {.callout-tip icon=false .fragment fragment-index=3 style="font-size: 75%;"}
## <span style="color:green">{{< fa circle-info >}}</span> `map()` Variants
By default, `map()` returns a `list`. However, if you know the specific datatype you want returned, you can use any of the [`map()` variants](https://purrr.tidyverse.org/reference/map.html) to return that explicitly.
:::
...
gapminder <- paths |> 
  set_names(basename) |> 
  map(readxl::read_excel) |> 
  list_rbind(names_to = "year") |> # <1>
  mutate(year = parse_number(year)) # <2>
gapminder
```
1. The name of each list element (the filename) will be saved as the variable `year`.
2. Extracting just the numeric part of the filename which is the actual year. 

. . . 

```{r}
write_csv(gapminder, "gapminder.csv") # <3>
```
3. Be sure to save your work so you can simply read in one file when working on this project in the future!
...
Since we don't actually care about the output (i.e. the printed datasets) and only want the files to be written, there's an even better function we can use for this task: `walk2()`. 
...
by_clarity <- by_clarity |> 
  mutate(
    plot = map(data, carat_histogram), # <1>
    path = str_glue("clarity-{clarity}.png") # <2>
  )
by_clarity
```
1. Creates a column called plot where each observation is the ggplot object for the corresponding data.
2. Overwrites the `path` column to specify the corresponding filepaths for each ggplot object in `plot`. 
...
walk2(
  by_clarity$path, # <1>
  by_clarity$plot, # <2>
  \(path, plot) ggsave(path, plot, width = 6, height = 6) # <3>
)
```
1. The first argument (the filepath) provided to our anonymous function below. 
2. The second argument (the ggplot object) provided to our anonymous function below. 
3. Our anonymous function, specifying the first two required arguments of `ggsave()`, as well as any additional (optional) argument(s) we'd like to specify (in this case, the width and height of the saved plot).
...
```{r}
#| code-line-numbers: false
tapply(diamonds$price, diamonds$cut, mean) # <1>
```
1. The first argument (`X`) is grouped by the second argument (`INDEX`) and the third argument (`FUN`) is applied. 
