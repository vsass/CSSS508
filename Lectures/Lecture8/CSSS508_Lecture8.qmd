...
## Data Frame Function Examples [{{< fa scroll >}}]{style="color:#99a486"} {.scrollable}
...
::: {.fragment}
![](histogram.png){.absolute right=0, top=25}
:::
:::
...
2. Write a function that takes a name as an input (i.e. a character string) and returns a greeting based on the current time of day. **Hint**: use a time argument that defaults to `lubridate::now()`. That will make it easier to test your function.
