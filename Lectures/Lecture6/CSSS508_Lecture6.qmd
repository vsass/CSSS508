...
x / 5 # <4> 
```
4. This is shorthand for: `x / c(5, 5, 5, 5)`
...
```{r}
flights |> 
  filter(month == c(1, 2)) |> # <5> 
  head(5)
```
5. A common mistake is to mix up `==` with `%in%`. This code will actually find flights in odd numbered rows that departed in January and flights in even numbered rows that departed in February. Unfortunately there’s no warning because `flights` has an even number of rows. 
...
## Summary Functions <span style="color:#99a486">{{< fa scroll >}}</span> {.scrollable} 
...
For Homework 6, you will fill in an Quarto template on our [course website](https://vsass.github.io/CSSS508/Homework/HW6/homework6.html) that walks you through the process of creating, accessing, and manipulating R data structures. Enter values in the qmd document and Render it to check your answers!
