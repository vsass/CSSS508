...
::: {.column width="50%"}
![1.](images/multiply_matrices.gif)
:::
...
To invert an invertible square matrix^[![](images/inverse_matrix.png)], use `solve()`.
...
This distinction leads us to the final data type, of which `data.frame`s and `tibble`s are a subset. 

::: aside
`tibble`'s additional advantages are only displaying the first 10 rows and only the columns that will comfortably fit within the parameters of your console space when called (`data.frame`will display all columns and a certain number of rows depending upon your default max setting - usually this is an overwhelming amount of output, aesthetical confusing,  and not very helpful).
:::
...
You can supply names to index into named vectors. This is the same as `x[[1]][[2]][["elt"]]`. 
