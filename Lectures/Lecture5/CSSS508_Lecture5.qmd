...
1. Go to the [Lecture 5 Homepage](https://vsass.github.io/CSSS508/Lectures/Lecture5/CSSS508_Lecture5_index.html) and click on the link `Religion & Income` under the Data section. 
  ...
  iv) Turn the variable `religion` and one other^[Hint: Created in the last step] into factors.
  v) Make a visualization in ggplot that 
  
```{r}
religion_income <- read_csv(file = "data/religion_income.csv")
religion_income_tidy <- relig_income |> 
...
ggplot(religion_income_tidy, aes(x = income, y = count, fill = income)) + 
  geom_bar(stat = "identity") + facet_wrap(religion ~ . )
```
