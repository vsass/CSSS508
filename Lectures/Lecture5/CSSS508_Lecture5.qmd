...
4. `names_prefix` is used to remove "wk" from the values of `week`
...
ggplot(relig_summary, aes(x = tvhours, y = relig)) + # <7>
  geom_point()
```
...
9. Ordered factors imply a strict ordering and equal distance between levels: the first level is “less than” the second level by the same amount that the second level is “less than” the third level, and so on.
...
::: {.fragment}
If the offset is in seconds you'll want to use `as_date_time()` and if it's in days use `as_date()` for the conversion. 
...
17. The `+10:30` offset is the difference relative to `UTC` (aka Greenwich Mean Time) which is `2024-06-01 16:00:00 UTC` in this example.
...
1. Go to the [Lecture 5 Homepage](https://vsass.github.io/CSSS508/Lectures/Lecture5/CSSS508_Lecture5_index.html) and click on the link `Religion & Income` under the Data section. 
  ...
  iv) Turn two of the variables into factors^[Hint: Which two variables are categorical?].
  v) Excluding the income category `Don't know/refused`, which religion-income combination has the largest frequency^[You can determine this by summarizing the data or creating a visualization of the data.]?

## Answers 

Question 1.i: Click *File* > *Save* to download this data to the same folder where your source document^[`R` Script or `.qmd`] will be saved for this lab.
...
Question 1.iii: Pivot your data to make it tidy^[There should be 3 columns in your final dataset].
...
Question 1.iv: Turn two of the variables into factors^[Hint: Which two variables are categorical?].
...
Question 1.v: Excluding the income category `Don't know/refused`, which religion-income combination has the largest frequency^[You can determine this by summarizing the data or creating a visualization of the data.]?
...
## Homework 5 <span style="color:#99a486">{{< fa scroll >}}</span> {.scrollable} 
