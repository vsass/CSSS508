...
The mean petal length is `r mean(iris$Petal.Length)` but its median petal length is `r median(iris$Petal.Length)`. It's range is `r max(iris$Petal.Length) - min(iris$Petal.Length)` which additionally suggests a certain degree of spread.  
