...
billboard_top3_month_viz <- billboard_tidy_date |> 
  ...
  mutate(peak_weeks = function(variable), # <4>
         month_peak = ifelse(variable > 0, variable, NA), # <5>
         .by = c(month, artist, track)) |> 
  filter(function(month_peak == "value"), # <6>
         .by = c(track, artist)) 

library(ggrepel) # <7>
ggplot(billboard_top3_month_viz, 
       aes(variable, variable, group = variable, color = variable)) + # <8> 
  function + # <9>
  geom_label_repel(data = billboard_top3_month_viz |> function(variable, by = track), # <10>
                   mapping = aes(label = variable)) # <11> 
```
...
4. Get a total of all the weeks a particular song was in the top 3 in a particular month
5. Need to create a variable that reflects when a particular song charted in the top 3 and `NA` otherwise
6. In order to get the entire trajectory of a song we can't simply filter for the month when it peaked. Then we'd only be able to plot that snippet of its trajectory. Hint: Which function returns `TRUE` if even 1 element it's given is `TRUE`?
7. Load if you want to add labels
8. You want to visualize the ranking trajectory of a song over time. Hint: `group` is the variable you want to visualize.
9. What geometry would be appropriate here?
10. To properly label this plot you'll need to subset the data, otherwise it will try to plot a label for every date available. 
11. Which variable's text are you trying to label?
