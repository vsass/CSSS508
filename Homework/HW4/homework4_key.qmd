...
Use `left_join` to add weather data at departure to the subsetted data^[Hint: What variables could you match on that equal `time_hour`?]. Calculate the mean temperature by `month` at departure (`temp`) across all flights.
...
Investigate if there is a relationship between departure delay (`dep_delay`) and precipitation (`precip`) in the full dataset. Is the relationship different between JFK, LGA, and EWR? I suggest answering this question by making a plot and writing down a one-sentence interpretation.
