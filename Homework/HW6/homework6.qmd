...
#### Question 1: Vectors
  i. `LETTERS`^[`LETTERS` and `letters` are built-in vectors in `R` that you can use without loading any external packages.] contains the 26 capital letters in order. Use `LETTERS` and `[ ]` to create a vector called `vec_char` of the first 10 capital letters.
